    ...
    defaultValues: {
      client: {
        drive: {
          ...
          send_file_request_notification:
            data.client.drive?.send_file_request_notification ?? true,
...
function FileRequestNotificationsPanel() {
  return (
    <SettingsPanel
      title={<Trans message="File request notifications" />}
      description={
        <Trans message="Send a notification to user when someone uploads a file to one of their file requests." />
      }
    >
      <FormSwitch name="client.drive.send_file_request_notification">
        <Trans message="Enable notifications" />
      </FormSwitch>
    </SettingsPanel>
  );
