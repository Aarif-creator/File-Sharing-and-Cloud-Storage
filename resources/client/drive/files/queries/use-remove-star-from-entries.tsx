import {driveBaseKey} from '@app/app-queries';
import {unstarEntries} from '@app/gen/files';
import {showHttpErrorToast} from '@common/http/errors/show-http-error-toast';
import {queryClient} from '@common/http/query-client';
import {toast} from '@shadcn/toast/toast';
import {useMutation} from '@tanstack/react-query';
import {Trans} from '@ui/i18n/trans';
import {FirstParam} from '@ui/utils/ts/extract-params';

export function useRemoveStarFromEntries() {
  return useMutation({
    mutationFn: (body: FirstParam<typeof unstarEntries>) => unstarEntries(body),
    onSuccess: (response, body) => {
      queryClient.invalidateQueries({queryKey: driveBaseKey});
      toast.success(
        <Trans
          message="Removed star from [one 1 item|other :count items]"
          values={{
            count: body.entryIds.length,
          }}
        />,
      );
    },
    onError: err =>
      showHttpErrorToast(err, <Trans message="Could not remove star" />),
  });
}
