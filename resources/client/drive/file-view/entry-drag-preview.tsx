...
export function EntryDragPreview() {
  ...
  useDragMonitor({
    ...
    onDragStart: (e, dragTarget) => {
      const target = dragTarget as ConnectedDraggable<DriveEntry[]>;
      if (target) {
        setState({
  ...
  if (state.entries && state.e) {
    preview = (
      <div
        ...
        className="pointer-events-none fixed top-0 left-0 isolate"
      >
  ...
  ({entry, points, index, state}: EntryPreviewProps) => {
    ...
    return (
      <m.div
        ...
        className={clsx(
          'absolute max-h-12 rounded-card border border-primary/25 bg whitespace-nowrap',
          index < 2 && 'shadow-sm',
          index === 0 && 'top-0 left-0 z-10',
          index > 0 && 'top-1.5 left-1.5',
        )}
      >
        {itemCount > 1 && index === 0 && <EntryCount count={itemCount} />}
        <div className="flex h-full items-center justify-center gap-2.5 overflow-hidden rounded-card bg-primary/10 px-4 text-sm">
          <FileTypeIcon type={entry.type} />
          <div className="flex-auto truncate">{entry.name}</div>
        </div>
      </m.div>
    );
  },
...
const EntryCount = memo(({count}: EntryCountProps) => {
  return (
    <m.div
      ...
      className="absolute -top-1.5 -right-1.5 z-10 z-30 flex h-5 w-5 items-center justify-center rounded-full bg-destructive text-sm font-bold text-white shadow-lg"
    >
      {count}
    </m.div>
  );
